...
Two subtle CSS fixes worth flagging: `.jh-chip { line-height: 1.2 }` is now set explicitly (without it, chips inherited body's 1.55 and the row-to-row vertical gap read cramped against the wider horizontal padding); and the tech modal extends to its content height instead of scrolling internally — the outer `.jh-modal` overlay is the only scroller, so body text is always reachable regardless of viewport height. The trade-off is the sidenav no longer feels pinned; scroll-spy in [components/TechModalBody.tsx](components/TechModalBody.tsx) listens to the outer modal's scroll events via a `findOuterScroller()` walk to the `.jh-modal` ancestor.

Frontend-design pass shipped on top of those fixes: (1) `components/FadeUp.tsx` was rewritten as progressive enhancement — SSR renders `<div class="jh-fadeup">` with no styles, and JS adds `jh-fadeup--prime` in a `useLayoutEffect` before paint, then `is-in` via IntersectionObserver. If JS fails to hydrate, all four main sections stay visible. (2) Body sans swapped from Inter to **IBM Plex Sans** (`--jh-sans`) — pairs warm-serif Newsreader with a slightly-technical sans that matches the project-controls identity. (3) Section eyebrows breathe wider — `.jh-kicker` letter-spacing transitions from `0.16em` (primed) to `0.22em` (in-view) over 1.2s when the FadeUp wrapper enters view; reduced-motion pins it at 0.16em. (4) `.jh-copyright` flex no-op removed (single child); ultrawide breakpoint lowered from 2600 → 2560 so 27" 1440p monitors get the bigger sidebar tier. (The paper-grain `body::before` overlay that originally shipped in this pass was removed during the rem refactor — the `feTurbulence` + `mix-blend-mode: multiply` combo wasn't rendering visibly in Firefox, so the layer was doing nothing useful.)
...
**Scaling:** `:root { font-size: 110% }` makes 1rem = 17.6px (assuming the 16px browser default), so the design renders 10% larger than its 16-based spec. Every sizing/spacing/typography value in `globals.css` is expressed in `rem` to follow the root; only hairlines, border widths, focus-ring outlines, shadow geometry, and `border-radius: 999px` pill sentinels stay in `px`. New rules should follow the same convention — write `Xpx` only when the value should NOT scale with the root.
